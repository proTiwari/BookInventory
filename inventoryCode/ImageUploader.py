...
def ImageUploader(frontCoverImageUrl):
    # fileName3 = "front.jpg"
    ...
    if not frontCoverImageList:
        errorList.append(f'No match for {frontCoverImageUrl} found')
    ...
    #     blob3 = bucket.blob(f'books_database/{id}/supportingImage/{fileName3}')
    #     blob3.upload_from_filename(fileName3)
    ...
    blob1 = bucket.blob(f'books_database/{id}/frontCoverImage/{fileName1}')
    blob1.upload_from_filename(fileName1)
    down1 = blob1.public_url
    frontUrlList.append(down1)

    # blob2 = bucket.blob(f'books_database/{id}/backCoverImage/{fileName2}')
